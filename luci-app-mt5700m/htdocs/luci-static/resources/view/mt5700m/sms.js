...
function decodePdu(pdu, index) {
	try {
		if (!/^(?:[0-9a-f]{2})+$/i.test(pdu)) return null;
		var p = 0, smscLen = parseInt(pdu.substring(p, p + 2), 16); p += 2 + smscLen * 2;
		var first = parseInt(pdu.substring(p, p + 2), 16); p += 2;
		// Only SMS-DELIVER is an incoming message, not SMS-SUBMIT or a status report.
		if (!Number.isFinite(first) || (first & 3) !== 0) return null;
		...
		if ((toa & 0x70) === 0x50) number = decodeGsm7(numberHex, Math.floor(digits * 4 / 7), 0);
		p += 2;
		...
		if (!Number.isFinite(udl) || ud.length < ((dcs & 0x0c) === 0 ? Math.ceil(udl * 7 / 8) : udl) * 2) return null;
		if ((dcs & 0x0c) !== 0 && (dcs & 0x0c) !== 8) return null;
		if (first & 0x40) {
			headerBytes = parseInt(ud.substring(0, 2), 16) + 1;
			if (!Number.isFinite(headerBytes) || headerBytes * 2 > ud.length) return null;
			var h = 2;
			while (h < headerBytes * 2) {
				...
				if (!Number.isFinite(len) || h > headerBytes * 2) return null;
...
function parseMessages(raw) {
	...
	messages.forEach(function(msg) {
		...
		var key = msg.number + ':' + msg.concat.ref + ':' + msg.concat.total + ':' + msg.date.substring(0, 10);
	...
	Object.keys(groups).forEach(function(key) {
		...
		if (parts.length !== first.concat.total || !parts.every(function(part, i) { return part.concat.seq === i + 1; })) {
			merged = merged.concat(parts); return;
		}
...
return view.extend({
	...
	load: function() { return Promise.all([
		controls.exec('/usr/sbin/mt5700m-read',['sms-list']).then(function(res){return res.stdout||'';},function(){return '';}),
		controls.exec('/usr/sbin/mt5700m-read',['sms-info']).then(function(res){return res.stdout||'';},function(){return '';})
	]); },
	...
	render:function(data){
		...
		if (Number(info.storage[2]) > 0 && Number(info.storage[1]) / Number(info.storage[2]) >= 0.8)
			ui.addNotification(null,E('p',{},_('SMS storage is nearly full. Review and delete only messages you no longer need; existing messages are never deleted automatically.')),'warning');
